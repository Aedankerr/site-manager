# Aangepaste secties

Met aangepaste secties kunt u inhoud toevoegen die niet past bij de ingebouwde secties.

## Een aangepaste sectie aanmaken

1. Open **Instellingen** → tabblad **Aangepaste secties**
2. Klik op **Aangepaste sectie toevoegen**
3. Voer een sectienaam in
4. Kies een lay-outtype (zie hieronder)
5. Kies optioneel een **sectiepictogram** (ster, boek, link, wereldbol, hart, onderscheiding, aktetas) — weergegeven naast de kop
6. Sla op — de nieuwe sectie verschijnt in uw CV

## Lay-outtypen

| Lay-out | Geschikt voor |
|---------|---------------|
| **2-koloms raster** | Gekoppelde items zoals talen & vaardigheidsniveau, tools & ervaringsniveau |
| **3-koloms raster** | Compacte items zoals prijzen, publicaties of korte referenties |
| **Verticale lijst** | Opeenvolgende items met optionele links, zoals vrijwilligerswerk of lidmaatschappen |
| **Kaartraster** | Rijke items met titel, ondertitel, beschrijving en link — zoals portfoliostukken |
| **Sociale links** | Platformspecifieke links met pictogrammen (LinkedIn, GitHub, Twitter/X, YouTube, Instagram, Dribbble, Behance, Website, E-mail, Telefoon of Aangepast) |
| **Opsommingstekens** | Gegroepeerde opsommingslijsten, vergelijkbaar met hoogtepunten bij werkervaring — ideaal voor belangrijke prestaties of competenties |
| **Vrije tekst** | Platte tekst met behouden regelovergangen — vergelijkbaar met de Over mij-sectie, bruikbaar voor motivatiebrieven, persoonlijke verklaringen of andere vrije inhoud |

## Titel verbergen

Voor raster-, lijst-, kaart- en opsommingslay-outs heeft elk item een selectievakje **"Titel verbergen"**. Wanneer ingeschakeld, wordt het item weergegeven zonder titelkop — handig wanneer de inhoud voor zich spreekt.

## Items in aangepaste secties beheren

Klik op **Items beheren** bij een aangepaste sectie om items toe te voegen, te bewerken, te herschikken of te verwijderen binnen die sectie.

!!! note "Opmerking"
    Er is geen harde limiet voor het aantal aangepaste secties. Maak er zoveel aan als u nodig heeft. U kunt ook het lay-outtype van een aangepaste sectie wijzigen nadat u deze heeft aangemaakt — bewerk simpelweg de sectie en selecteer een andere lay-out.
