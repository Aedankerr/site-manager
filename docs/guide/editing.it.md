...
Quando viene impostato un logo, appare un interruttore: **"Sincronizza il logo su tutte le esperienze di [Azienda]"**. Quando attivato:
...
Ogni certificazione ha un **nome**, un **ente certificatore**, una **data di rilascio** e, opzionalmente, una **data di scadenza** e un **ID credenziale**.
...
Le competenze sono organizzate in **categorie** (ad es. "Linguaggi di programmazione", "Strumenti e piattaforme"). Ogni categoria contiene etichette di singole competenze. Le categorie possono avere un'**icona** selezionata da un elenco predefinito: code, server, database, cloud, settings, users, briefcase, cpu, layers o default. Se non viene scelta alcuna icona, il sistema ne rileva automaticamente una in base al nome della categoria (ad es. una categoria chiamata "Programming" ottiene automaticamente l'icona code).
...
Nella maggior parte delle sezioni, gli elementi possono essere riordinati trascinandoli. L'ordine viene salvato automaticamente.
