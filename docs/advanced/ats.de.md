# ATS-Kompatibilität

CV Manager erzeugt ATS-freundliche (Applicant Tracking System) Ausgaben:

- **Schema.org-Markup** — strukturierte Daten, die ATS-Systeme parsen können (Person, OrganizationRole, EducationalOccupationalCredential usw.)
- **Semantisches HTML** — korrekte Überschriftenhierarchie, Article-Elemente und Listen
- **Versteckter ATS-Block** — eine Klartext-Version Ihres Lebenslaufs ist in die Seite eingebettet, für Parser, die gestyltes HTML nicht verarbeiten
- **Saubere Druckausgabe** — kein visuelles Durcheinander, korrekte Inhaltshierarchie

Es ist keine spezielle Konfiguration erforderlich — die ATS-Optimierung ist immer aktiv.
