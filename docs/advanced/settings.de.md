...
Steuern Sie unter **Settings → Advanced → Search Engine Indexing**, wie Suchmaschinen mit Ihrem öffentlichen Lebenslauf interagieren:
...
Um Suchmaschinen das Crawlen versionierter URLs zu erlauben, aktivieren Sie **Index Versioned URLs** unter **Settings → Advanced**. Diese Einstellung ist unabhängig von der oben genannten Suchmaschinenindexierungs-Option, die nur die Hauptseite `/` betrifft.

## Tracking-Code

Fügen Sie Analytics-Tracking-Code (Google Analytics, Matomo, Plausible etc.) unter **Settings → Advanced → Tracking Code** ein. Der Code wird nur in die öffentlichen Lebenslauf-Seiten eingefügt — nicht in die Admin-Oberfläche.
